...
Downloads new running FIT files from Garmin Connect and:
  ...
  4. Writes latest_run.json to the GitHub repo (fetchable via raw URL)
...
# GitHub config — token injected via env var in Actions
...
def push_to_github(json_bytes):
    """Write latest_run.json directly into the GitHub repo via API."""
    if not GITHUB_TOKEN:
    ...
    if resp.status_code in [200, 201]:
        ...
        log.info("GitHub: latest_run.json updated.")
...
def sync():
    ...
    # Sort oldest first so latest_run.json ends up as the newest
    ...
    for activity in activities:
        ...
        try:
            ...
            # 2. Google Drive — raw FIT
            upload_to_gdrive(gdrive, fit_data, fit_filename)

            # 3. Google Drive — dated JSON
    ...
    if github_raw_url:
        log.info("=" * 60)
        log.info("LATEST RUN — GITHUB RAW URL (use this with Claude):")
        log.info("%s", github_raw_url)
        log.info("=" * 60)
