...
Fixes in v8:
  - Athlete name anonymised as Athlete_3B974C
  - FIT extraction from ZIP wrapper (Garmin API returns .fit.zip)
  - Full lap data population verified
...
ATHLETE_ID      = "Athlete_3B974C"   # anonymised — do not change
...
# ── FIT extraction (handles both raw FIT and ZIP wrapper) ─────────────────────
def extract_fit(data: bytes) -> bytes:
    """Extract raw FIT bytes — handles both plain FIT and ZIP-wrapped FIT."""
    # Check for ZIP magic bytes
    if data[:2] == b'PK':
        log.info("Detected ZIP wrapper — extracting FIT file...")
...
def build_json_summary(raw_download, activity):
    """Build full JSON summary. raw_download may be ZIP or raw FIT."""
    ...
    if not laps or not records:
        log.warning("No lap/record data found — FIT may be corrupt or unsupported")
...
def sync():
    ...
    for activity in activities:
        ...
        try:
            ...
            # Extract raw FIT for Dropbox/Drive archive
            fit_data = extract_fit(raw_download)
