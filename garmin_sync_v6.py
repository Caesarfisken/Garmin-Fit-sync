...
Downloads new running FIT files from Garmin Connect and:
  ...
  2. Uploads raw FIT + dated JSON summary to Google Drive
  3. Overwrites latest_run.json in Google Drive (fixed public URL for Claude)
...
# This filename is fixed and always contains the latest run
...
def build_json_summary(fit_data, activity):
    ...
    for li in range(len(laps)):
        ...
        dist = lap.get(9, 0) / 100; elapsed = lap.get(7, 0) / 1000
        if dist < 50: continue
        ...
        laps_out.append({
            "lap": li + 1, "distance_m": round(dist, 1), "duration_s": round(elapsed, 1),
            "pace_min_km": pace_str, "pace_decimal": round(pace_min, 3) if pace_min else None,
            "avg_hr_bpm": avg_hr, "hr_zone": get_zone(avg_hr) if avg_hr else None,
            "hr_zone_pct": get_zone_pct(avg_hr) if avg_hr else None,
            "cadence_spm": avg_cad, "vertical_oscillation_mm": avg_vo,
            "ground_contact_time_ms": avg_gct, "vertical_ratio_pct": avg_vr,
    ...
    return {
        ...
        "athlete": {
            "name": "Julius Schmidt", "rhr_bpm": RHR, "mhr_bpm": MHR,
            "hrr_bpm": HRR, "vo2max": 57,
            "zones": {
                "Z1": "102-115 bpm (50-60% HRR)", "Z2": "115-128 bpm (60-70% HRR)",
                "Z3": "128-141 bpm (70-80% HRR)", "Z4": "141-154 bpm (80-90% HRR)",
        ...
        "baseline_reference": {
            ...
            "leg_spring_stiffness_kn_m": 8.8, "form_power_pct": 32.2
        },
        "laps": laps_out
    }
...
def make_file_public(service, file_id):
    """Make a file publicly readable via link."""
    service.permissions().create(
        fileId=file_id,
        body={"type": "anyone", "role": "reader"}
    ).execute()
    file_info = service.files().get(fileId=file_id, fields="webContentLink, id").execute()
...
def sync():
    ...
    # Process most recent activity last so latest_run.json = newest run
    activities = sorted(activities, key=lambda a: a.get("startTimeLocal", ""))

    latest_url = state.get("latest_run_url")
    new_count = 0
    ...
    for activity in activities:
        ...
        try:
            ...
            # Google Drive: overwrite latest_run.json with fixed public URL
            log.info("Updating latest_run.json ...")
            latest_id = upload_to_gdrive(gdrive, json_bytes, LATEST_RUN_FILENAME, "application/json")
            latest_url = make_file_public(gdrive, latest_id)
            log.info("latest_run.json public URL: %s", latest_url)
    ...
    state["latest_run_url"] = latest_url
    save_state(state)

    if latest_url:
        log.info("="*60)
        log.info("LATEST RUN URL (share with Claude):")
        log.info("%s", latest_url)
        log.info("="*60)
